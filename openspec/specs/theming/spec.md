...
## Purpose
Runtime-selectable theme system (Liquid Glass / Neuromorphic) with light/dark token sets pushed into Slint root properties, so all panels and the alarm overlay render in the active variant without recompilation. Mode selection (`Manual-Light` / `Manual-Dark` / `Follow-Bedtime`) persists across reboot.
