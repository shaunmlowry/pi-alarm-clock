...
- `ShiftForward`: advance `next_fire` to the **next non-holiday date** at the same scheduled wall-clock time, repeating the skip until a non-holiday date is found (so a multi-day holiday weekend shifts the alarm to the first non-holiday day).
